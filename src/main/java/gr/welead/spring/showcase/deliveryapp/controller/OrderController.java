...
public class OrderController extends BaseController<Order, OrderResource>{
    ...
    public ResponseEntity<ApiResponse<List<OrderDetailsReportByCustomerId[]>>> getOrdersByCustomer(@PathVariable Long customerId) {
        ...
        List<OrderDetailsReportByCustomerId[]> orders = orderService.findOrdersDetailsByCustomerId(customer.getId());
        logger.info("orders {} ", orders);

        return ResponseEntity.ok(
                ApiResponse.<List<OrderDetailsReportByCustomerId[]>>builder()
                        .data(orders)
                        .build()
        );
    }
